...
## 概述
当用户提供 PDF 报价单或资源清单时，先用本技能做“第一步抽取”，通过 Azure Document Intelligence 识别文本并提取 VM 报价所需字段，输出和 `input-excel-extraction` 对齐的标准中间数据。
...
- 输出 CSV 供 `global-region-mapping` 等后续 skill 链式消费。
...
### 使用自定义模型
`python .github/skills/input-pdf-extraction/scripts/extract_pdf_inputs.py --input-pdf "input/sample_input.pdf" --model-id "prebuilt-layout" --output "output/extracted_inputs_from_pdf.csv"`

### 过滤抽取（按 provider / resource_type）
`python .github/skills/input-pdf-extraction/scripts/extract_pdf_inputs.py --input-pdf "input/sample_input.pdf" --profile all_resources --provider aws --resource-type vm --output "output/aws_vm_rows_from_pdf.csv"`
