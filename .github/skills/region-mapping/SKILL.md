...
### 单条查询
`python skills/region-mapping/scripts/region_mapping.py --location "Singapore"`

### 批量映射
`python skills/region-mapping/scripts/region_mapping.py --input-file "input/locations.csv" --column "region" --output "output/mapped_results.csv"`
...
- `--mapping-file`：映射表路径；未传时优先使用 `skills/region-mapping/assets/get_regions.xlsx`，其次 `data/get_regions.xlsx`。
